...
// Start der PV Prognose Sektion
  ...
  if ( ChaTm > 0 && (ChaTm*2) <= pvfc.length && batsoc >= batminlimit) {
    ...
    for (let k = 0; k < pvfc.length; k++) {
      get_wh = get_wh + (((pvfc[k][1]/2)-((pvlimit+grundlast)/2))*(pvfc[k][2]/30)) // wieviele Wh Überschuss???
    }
    ...
    if (ChaEnrg > get_wh && ChaEnrg > 0 && ChaTm > 0){
      ...
      pvlimit = pvlimit - ((ChaEnrg - get_wh)/ChaTm) //virtuelles reduzieren des pvlimits
      get_wh = ChaEnrg // sprungpunkt in Scenario 5 
    }
    if (debug == 1){console.log(pvlimit)}
    if (debug == 1){console.log(get_wh)}
    //Scenario 5
