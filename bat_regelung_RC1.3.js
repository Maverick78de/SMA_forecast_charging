...
// Start der PV Prognose Sektion
  ...
  if (debug == 1){console.log(latesttime)}
  ...
  if ( ChaTm > 0 && (ChaTm*2) <= pvfc.length && batsoc >= batminlimit) {
    ...
    if (get_wh >= ChaEnrg && ChaEnrg > 0){
      ...
      if (debug == 1){console.log('100' + '-' + (pvlimit-grundlast) + '+' + cur_power_out)}
      if (debug == 1){console.log(power_ac + '+' + current_pwr_diff)}
      ...
      if (get_wh < ChaEnrg && ChaEnrg > 0 && ChaTm > 0){
