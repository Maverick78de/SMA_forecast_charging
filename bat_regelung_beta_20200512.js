...
var update = 15, /*Update interval in sek, 15 ist ein guter Wert*/
    ...
    pv_ac_pwr = 11500, /*Leistung der PV Wechselrichter (Max AC Power) in W*/
...
// Start der PV Prognose Sektion
  ...
  if ( ChaTm > 0 && (ChaTm*2) <= pvfc.length && batsoc >= batminlimit) {
    ...
    if (get_wh >= ChaEnrg && ChaEnrg > 0){
      ChaTm = pvfc.length/2      
      var current_pwr_diff = (pvlimit-grundlast)+cur_power_out 
      if (PVDevType < 9300){
        current_pwr_diff = 100-(pvlimit-grundlast)+cur_power_out //bleibe 100W unter dem Limit (PV-WR Trigger)
      }
    ...
    for (let h = 0; h < (ChaTm*2); h++) {
      ...
      if ( (compareTime(pvfc[h][1], pvfc[h][2], "between")) || pvwrlimit < 100 ){ 
        bms = 2289;
        maxchrg = max_pwr;
        maxdischrg = maxdischrg_def,
        SpntCom = SpntCom_def,
